...
#to_test <- full_imp[-c(1:nrow(dt.test)), ]
...
write.csv(predrf, file = "../submissions/rf_test.csv", row.names = F) 




# evaluate feature importance
obj <- randomForest(OutcomeType~., data = to_train, 
             mtry = 4, ntree = 100, do.trace = T, importance = TRUE)
obj$importance
importance(obj$importance, type=1)


head(to_train$AnimalType)
unique(to_train$Breed)
